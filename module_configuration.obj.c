...
d_define_method(module_configuration, fill_resources)(struct s_object *self, struct s_object *environment) {
  ...
  if (d_pomodoro_fullscreen) {
    d_assert(checkbox_fullscreen = d_call(module_configuration_attributes->ui_factory, m_ui_factory_get_component, resources_container, "fullscreen_checkbox"));
    d_call(checkbox_fullscreen->uiable, m_checkbox_set_checked, d_true);
  }
  for (index = 0; v_configuration_resolutions[index]; ++index)
    if ((current_label =
           d_call(module_configuration_attributes->ui_factory, m_ui_factory_new_label, d_ui_factory_default_font_id, d_ui_factory_default_font_style,
                  v_configuration_resolutions[index]))) {
  ...
  for (index = 0; v_configuration_languages[index]; ++index)
    if ((current_label =
           d_call(module_configuration_attributes->ui_factory, m_ui_factory_new_label, d_ui_factory_default_font_id, d_ui_factory_default_font_style,
                  v_configuration_languages[index]))) {
...
d_define_class(module_configuration) {d_hook_method(module_configuration, e_flag_private, fill_resources),
...
void p_module_configuration_apply(struct s_object *self, void **parameters, size_t entries) {
  ...
  if ((width != d_pomodoro_width_window) || (height != d_pomodoro_height_window) || (fullscreen != d_pomodoro_fullscreen) ||
    ...
    d_try{
          if ((stream_configuation =
                 d_call(module_configuration_attributes->resources_json, m_resources_get_stream, d_factory_configuration, e_resources_type_common))) {
