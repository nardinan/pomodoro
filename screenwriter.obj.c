                      ...
                      while (((index_next_assignment = p_screenwriter_get_dialog(json, next_entry_ID)) != d_screenwriter_not_found) &&
                        ...
                        if (f_string_strcmp(current_type, v_screenwriter_types[e_screenwriter_type_set]) == 0) {
                          if ((current_set = (struct s_screenwriter_set *) d_malloc(sizeof(struct s_screenwriter_set)))) {
                            ...
                            if ((!d_call(json, m_json_get_string, &next_entry_ID, "sds", "dialogs", index_next_assignment, "next")) || (!next_entry_ID)) {
                              d_err(e_log_level_ever, "missing 'next' filed in dialog[%d], dialog could be compromised", index_next_assignment);
                              break;
                            }
                          } else
...
d_define_method(screenwriter, get)(struct s_object *self, const char *ID) {
  ...
  d_foreach(&(screenwriter_attributes->dialogs), current_entry, struct s_screenwriter_entry)if (f_string_strcmp(current_entry->ID, ID) == 0)
      break;
...
d_define_method(screenwriter, update)(struct s_object *self) {
      ...
      if (!(intptr_t) d_call(current_character, m_character_is_speaking, NULL)) {
        if (screenwriter_attributes->current_entry->options.fill > 0) {
          ...
          if (bubble_attributes->last_value != d_bubble_no_value)
            d_foreach(&(screenwriter_attributes->current_entry->options), current_option, struct s_screenwriter_option)if (bubble_attributes->last_value ==
                                                                                                                           current_option->ID) {
                d_foreach(&(current_option->values), current_set, struct s_screenwriter_set)d_call(screenwriter_attributes->collector,
                                                                                                   m_collector_add_entry_double, current_set->key,
                                                                                                   current_set->value);
  ...
  if ((!screenwriter_attributes->current_entry) && (next_choice)) {
    ...
    if ((current_character = d_call(screenwriter_attributes->puppeteer, m_puppeteer_get_character, screenwriter_attributes->current_entry->actor))) {
      ...
      d_foreach(&(screenwriter_attributes->current_entry->options), current_option, struct s_screenwriter_option)d_call(character_attributes->bubble,
                                                                                                                        m_bubble_add_option,
                                                                                                                        current_option->content[screenwriter_attributes->language],
                                                                                                                        current_option->ID);
    }
  }
  return self;
...
d_define_class(screenwriter) {d_hook_method(screenwriter, e_flag_public, set_language),
