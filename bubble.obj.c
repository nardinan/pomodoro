...
struct s_object *
f_bubble_new(struct s_object *self, struct s_object *factory, unsigned int red, unsigned int green, unsigned int blue, unsigned int alpha, int font_style) {
    ...
    while (f_string_strlen(head_pointer) > d_bubble_characters_per_line) {
      ...
      if ((current_component = (struct s_bubble_component *) d_malloc(sizeof(struct s_bubble_component)))) {
        ...
        if ((current_component->component = f_label_new(d_new(label), current_component->content, selected_font, environment))) {
          ...
          d_call(current_component->component, m_drawable_set_maskRGB, bubble_attributes->mask_R, bubble_attributes->mask_G, bubble_attributes->mask_B);
...
d_define_method(bubble, skip)(struct s_object *self) {
  ...
  if (!bubble_attributes->current_element)
    if ((bubble_attributes->current_element = (struct s_bubble_message *) bubble_attributes->messages.head)) {
      ...
      if (bubble_attributes->current_element->track) {
        d_call(bubble_attributes->current_element->track, m_track_set_volume, (int) (d_track_default_volume * d_pomodoro_general_volume));
  ...
  if (bubble_attributes->current_element) {
    if ((selected_font =
           (TTF_Font *) d_call(bubble_attributes->factory, m_factory_get_font, bubble_attributes->current_element->font_ID, bubble_attributes->font_style,
                               &font_height))) {
  ...
  d_foreach(&(bubble_attributes->components), current_component, struct s_bubble_component) {
    ...
    if (((struct s_list_node *) current_component)->back == NULL) {
      ...
      if (bubble_attributes->drawables[e_uiable_component_top])
        d_call(current_component->component, m_uiable_set, bubble_attributes->drawables[e_uiable_component_top], e_uiable_mode_active, e_uiable_component_top);
    ...
    if (bubble_attributes->drawables[e_uiable_component_left])
      d_call(current_component->component, m_uiable_set, bubble_attributes->drawables[e_uiable_component_left], e_uiable_mode_active, e_uiable_component_left);
  ...
  d_foreach(&(bubble_attributes->components), current_component, struct s_bubble_component)d_call(current_component->component, m_drawable_set_dimension_w,
                                                                                                  maximum_width);
  return self;
}
d_define_method_override(bubble, draw)(struct s_object *self, struct s_object *environment) {
  ...
  struct s_bubble_component *current_component;
  double position_x, position_y, new_position_x, new_position_y, dimension_w, dimension_h, last_position_y, final_dimension_w, final_dimension_h;
  if (bubble_attributes->current_element) {
    d_call(self, m_drawable_get_position, &position_x, &position_y);
    d_foreach(&(bubble_attributes->components), current_component, struct s_bubble_component)if (current_component->component) {
    ...
    if (bubble_attributes->drawables[e_uiable_component_NULL]) {
      ...
      if (bubble_attributes->current_position == e_bubble_position_left) {
        ...
        new_position_x = position_x + final_dimension_w - dimension_w - 1;
      } else
        new_position_x = position_x + 1;
      new_position_y = last_position_y + final_dimension_h - d_uiable_default_border;
...
d_define_class(bubble) {d_hook_method(bubble, e_flag_public, set),
