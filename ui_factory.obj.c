...
void p_ui_factory_container_delete(struct s_uiable_container *container) {
    ...
    while ((current_container = (struct s_uiable_container *)container->childs.head)) {
        f_list_delete(&(container->childs), (struct s_list_node *)current_container);
    ...
    d_try {
        while ((d_call(json_ui, m_json_get_string, &string_supply, "sds", "root", index_container, "label"))) {
            ...
            if ((current_container = (struct s_uiable_container *)d_malloc(sizeof(struct s_uiable_container)))) {
                ...
                f_list_append(&(attributes->childs), (struct s_list_node *)current_container, e_list_insert_head);
                if ((starting_point = (t_json_starting_point *)d_call(json_ui, m_json_get_relative, NULL, "sd", "root", index_container))) {
                    ...
                    if ((current_container->uiable = d_call(self, m_ui_factory_new_container, resources_png_object, environment, boolean_supply))) {
                        d_call(current_container->uiable, m_drawable_set_position, position_x, position_y);
                        d_call(self, m_ui_factory_load_component, resources_png_object, environment, json_ui, current_container, starting_point);
                    }
                }
            } else
...
d_define_method(ui_factory, load_component)(struct s_object *self, struct s_object *resources_png_object, struct s_object *environment,
    ...
    while ((d_call(json_ui, m_json_get_string_relative, starting_point, &string_supply, "sds", "components", index_component, "label"))) {
        ...
        if ((current_component = (struct s_uiable_container *)d_malloc(sizeof(struct s_uiable_container)))) {
            ...
            f_list_append(&(current_container->childs), (struct s_list_node *)current_component, e_list_insert_head);
                ...
                if ((d_call(json_ui, m_json_get_string_relative, next_starting_point, &string_supply, "s", "kind"))) {
                    ...
                    if (scoped_keyword_list[index].kind) {
                        if (scoped_keyword_list[index].container) {
                            ...
                            if ((current_component->uiable = scoped_keyword_list[index].call(self, resources_png_object, environment, boolean_supply)))
                                d_call(self, m_ui_factory_load_component, resources_png_object, environment, json_ui, current_component, 
                                        next_starting_point);
                        } else {
                            ...
                            if ((current_component->uiable = scoped_keyword_list[index].call(self, resources_png_object, environment, 
                                            (unsigned int)font_id, (unsigned int)font_style, string_supply))) {
                    ...
                    if ((stream = d_call(resources_png_object, m_resources_get_stream, string_supply, e_resources_type_common)))
                        current_component->uiable = f_bitmap_new(d_new(bitmap), stream, environment);
...
d_define_method(ui_factory, load_uiable)(struct s_object *self, struct s_object *resources_png_object, struct s_object *environment, struct s_object *uiable,
    ...
    d_try {
        for (index_component = 0; index_component != e_uiable_component_NULL; ++index_component) {
            for (index_mode = 0; index_mode != e_uiable_mode_NULL; ++index_mode) {
                ...
                if (!(stream = d_call(resources_png_object, m_resources_get_stream_strict, buffer_path, e_resources_type_common))) {
                    ...
                    stream = d_call(resources_png_object, m_resources_get_stream_strict, buffer_path, e_resources_type_common);
                }
                if (stream)
                    if ((bitmap = f_bitmap_new(d_new(bitmap), stream, environment))) {
...
d_define_method(ui_factory, new_container)(struct s_object *self, struct s_object *resources_png_object, struct s_object *environment, t_boolean floatable) {
    d_using(ui_factory);
    struct s_exception *exception;
    ...
    d_try {
        ...
        if ((result = f_container_new(d_new(container), border_top, border_bottom, border_left, border_right, floatable)))
            d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, result, "container");
    } d_catch(exception) {
        d_exception_dump(stderr, exception);
        d_raise;
    } d_endtry;
...
d_define_method(ui_factory, new_list)(struct s_object *self, struct s_object *resources_png_object, struct s_object *environment) {
    d_using(ui_factory);
    struct s_exception *exception;
    ...
    d_try {
        ...
        if (!ui_factory_attributes->scroll_handler)
            if ((d_call(ui_factory_attributes->json_configuration, m_json_get_string, &string_supply, "sss", "ui", "scroll", "scroll")))
                if ((stream = d_call(resources_png_object, m_resources_get_stream, string_supply, e_resources_type_common)))
                    ui_factory_attributes->scroll_handler = f_bitmap_new(d_new(bitmap), stream, environment);
        if ((scroll = f_scroll_new(d_new(scroll), ui_factory_attributes->scroll_handler))) {
            ...
            d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, scroll, "scroll");
            if ((result = f_list_new(d_new(list), scroll))) {
                ...
                d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, result, "list");
            }
            d_delete(scroll);
        }
    } d_catch(exception) {
...
d_define_method(ui_factory, new_scroll)(struct s_object *self, struct s_object *resources_png_object, struct s_object *environment) {
    d_using(ui_factory);
    struct s_exception *exception;
    struct s_object *result = NULL;
    struct s_object *stream;
    ...
    d_try {
        ...
        if (!ui_factory_attributes->pointer_handler)
            if ((d_call(ui_factory_attributes->json_configuration, m_json_get_string, &string_supply, "sss", "ui", "scroll", "pointer")))
                if ((stream = d_call(resources_png_object, m_resources_get_stream, string_supply, e_resources_type_common)))
                    ui_factory_attributes->pointer_handler = f_bitmap_new(d_new(bitmap), stream, environment);
        if ((result = f_scroll_new(d_new(scroll), ui_factory_attributes->pointer_handler))) {
            ...
            d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, result, "scroll");
        }
    } d_catch(exception) {
        d_exception_dump(stderr, exception);
        d_raise;
    ...
    d_try {
        ...
        if (!ui_factory_attributes->checkbox_bitmap_checked)
            if ((d_call(ui_factory_attributes->json_configuration, m_json_get_string, &string_supply_checked, "sss", "ui", "checkbox", "checked")))
                if ((stream_checked = d_call(resources_png_object, m_resources_get_stream, string_supply_checked, e_resources_type_common)))
                    ui_factory_attributes->checkbox_bitmap_checked = f_bitmap_new(d_new(bitmap), stream_checked, environment);
    ...
    d_try {
        ...
        if ((current_font = d_call(director_attributes->factory, m_factory_get_font, font_id, font_style, &font_height)))
            if ((result = f_button_new(d_new(button), string_content, current_font, environment))) {
                ...
                label_attributes->alignment_x = (enum e_label_alignments)alignment_x;
                label_attributes->alignment_y = (enum e_label_alignments)alignment_y;
                d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, result, "button");
            }
    } d_catch(exception) {
        d_exception_dump(stderr, exception);
        d_raise;
    ...
    d_try {
        ...
        if ((current_font = d_call(director_attributes->factory, m_factory_get_font, font_id, font_style, &font_height)))
            if ((result = f_field_new_alignment(d_new(field), string_content, current_font, (enum e_label_background_formats)format,
                ...
                d_call(self, m_ui_factory_load_uiable, resources_png_object, environment, result, "field");
            }
    } d_catch(exception) {
        d_exception_dump(stderr, exception);
        d_raise;
...
d_define_method(ui_factory, get_component)(struct s_object *self, struct s_uiable_container *current_container, const char *label) {
    d_using(ui_factory);
    struct s_list *current_list = &(ui_factory_attributes->childs);
    struct s_uiable_container *result;
    if (current_container)
        current_list = &(current_container->childs);
...
d_define_method(ui_factory, delete)(struct s_object *self, struct s_ui_factory_attributes *attributes) {
    ...
    while ((current_container = (struct s_uiable_container *)attributes->childs.head)) {
        f_list_delete(&(attributes->childs), (struct s_list_node *)current_container);
        p_ui_factory_container_delete(current_container);
        d_free(current_container);
    }
    return NULL;
