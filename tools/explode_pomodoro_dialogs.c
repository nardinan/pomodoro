...
char author[d_string_buffer],
     name[d_string_buffer],
     location[d_string_buffer],
     description[d_string_buffer];
unsigned int lines;
enum e_dialog_line {
    e_dialog_code = 0,
    e_dialog_actor,
    ...
    if (stream) {
        ...
        while (current_dialog) {
            if ((current_dialog->italian_dialog) && (current_dialog->english_dialog)) {
                ...
                ++dialog;
                fprintf(stream, "(say \"%s\" (get_dialog dialogs language %d) \"%s_track%d\") ;preview: %s", current_dialog->actor, 
                        dialog, name, dialog, preview);
