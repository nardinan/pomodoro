...
d_define_method(stagecrafter, get_landscape)(struct s_object *self, const char *key) {
    ...
    d_foreach(&(stagecrafter_attributes->landscapes), current_landscape, struct s_stagecrafter_landscape)
        if (f_string_strcmp(current_landscape->label, key) == 0) {
...
d_define_method(stagecrafter, hide_landscapes)(struct s_object *self) {
    ...
    d_foreach(&(stagecrafter_attributes->landscapes), current_landscape, struct s_stagecrafter_landscape)
        d_call(current_landscape->landscape, m_landscape_hide, factory_attributes->environment);
    return self;
}
...
d_define_method(stagecrafter, linker)(struct s_object *self, struct s_object *script) {
    ...
    d_call(script, m_lisp_extend_environment, "stagecrafter_disable_item", p_lisp_object(script, e_lisp_object_type_primitive, 
                p_link_stagecrafter_disable_item));
...
d_define_class(stagecrafter) {
    d_hook_method(stagecrafter, e_flag_public, get_landscape),
