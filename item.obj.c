...
d_define_method_override(item, set_component)(struct s_object *self, char *label) {
  ...
  d_foreach(&(item_attributes->tracks), current_track, struct s_item_track)if (f_string_strcmp(current_track->label, label) == 0) {
...
d_define_class(item) {d_hook_method(item, e_flag_public, load),
