...
d_define_method(camera, update)(struct s_object *self, struct s_object *environment) {
  ...
  double camera_position_x, camera_position_y, current_distance, percentage_current_distance, percentage_movement, difference_seconds, t,
    final_position_x = 0.0, final_position_y = 0.0, final_position_z = 0.0;
  if (camera_attributes->reference)
  ...
  if (camera_attributes->distance_xy > 0.0) {
    ...
    current_distance =
      sqrt(d_math_square(camera_position_x - camera_attributes->destination_x) + d_math_square(camera_position_y - camera_attributes->destination_y));
    percentage_current_distance = (1.0 / camera_attributes->distance_xy) * (camera_attributes->distance_xy - current_distance);
    if ((percentage_movement = d_math_min(1.0, (camera_attributes->final_speed +
                                                ((camera_attributes->initial_speed - camera_attributes->final_speed) * (1.0 - percentage_current_distance))) *
      ...
      if ((camera_attributes->destination_x == final_position_x) && (camera_attributes->destination_y == final_position_y) &&
          (camera_attributes->destination_z == final_position_z))
        camera_attributes->distance_xy = 0;
      if ((camera_attributes->limit_x) && (final_position_z == 1.0)) {
      ...
      if ((camera_attributes->limit_y) && (final_position_z == 1.0)) {
...
d_define_class(camera) {d_hook_method(camera, e_flag_public, move_position),
