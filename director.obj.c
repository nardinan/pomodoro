...
t_boolean f_director_validator(struct s_object *self, double current_x, double current_y, double current_zoom, double *new_x, double *new_y, double *new_zoom) {
  ...
  if (character_attributes->action) {
    ...
    d_foreach(&(puppeteer_attributes->characters), current_character, struct s_puppeteer_character)if (current_character->character != self)
  ...
  if ((current_landscape = d_call(director_attributes->stagecrafter, m_stagecrafter_get_main_landscape, NULL)))
    if ((current_item = d_call(current_landscape, m_landscape_validator, self, current_x, current_y, new_x, new_y, new_zoom,
                               ...
                               environment_attributes->camera_origin_y[environment_attributes->current_surface], character_attributes->collidable)))
  ...
  switch (action->type) {
    ...
    case e_director_action_service_camera_follow:       /* key (character), destination_z */
      ...
      d_call(director_attributes->camera, m_camera_chase_reference,
             d_call(director_attributes->puppeteer, m_puppeteer_get_character, action->action.camera_follow.key), 0.0, action->action.camera_follow.position_y,
             action->action.camera_follow.position_z, factory_attributes->environment);
    default:
      break;
  }
  return result;
...
d_define_class(director) {d_hook_method(director, e_flag_public, set_language),
