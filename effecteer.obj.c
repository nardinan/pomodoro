...
d_define_method(effecteer, get_effect)(struct s_object *self, const char *key) {
  ...
  d_foreach(&(effecteer_attributes->components), current_effect, struct s_effecteer_effect)if (f_string_strcmp(current_effect->key, key) == 0)
      break;
...
d_define_method(effecteer, write_effect)(struct s_object *self, const char *key, char *message, double position_x, double position_y, double zoom, int font_ID,
                                         int font_style, int layer) {
  d_using(effecteer);
  struct s_factory_attributes *factory_attributes = d_cast(effecteer_attributes->factory, factory);
  ...
  if (index == d_effecteer_default_max_labels)
    index = 0;
  if (effecteer_attributes->labels[index].drawable) {
    d_call(factory_attributes->environment, m_environment_del_drawable, effecteer_attributes->labels[index].drawable, effecteer_attributes->labels[index].layer,
  ...
  switch (action->type) {
    case e_effecteer_action_add:            /* key (effect), label (effect), position_x, position_y, zoom, angle,
      ...
      result = d_call(self, m_effecteer_add_effect, action->key, action->parameters.action_add.label, action->parameters.action_add.position_x,
                      ...
                      action->parameters.action_add.angle,
                      (action->parameters.action_add.front) ? d_effecteer_default_front_layer : d_effecteer_default_back_layer);
      break;
    case e_effecteer_action_play:           /* key (effect), label (effect), fade_in (time), fade_out (time), volume, loop */
      d_log(e_log_level_medium, "action [play] (effect %s (%s) | fade_in %d | fade out %d | volume %d | %s)", action->key, action->parameters.action_play.label,
            action->parameters.action_play.fade_in, action->parameters.action_play.fade_out, action->parameters.action_play.volume,
            (action->parameters.action_play.loop) ? "loop" : "single");
...
d_define_method(effecteer, delete)(struct s_object *self, struct s_effecteer_attributes *attributes) {
  ...
  for (index = 0; index < d_effecteer_default_max_labels; ++index)
    if (attributes->labels[index].drawable) {
      d_call(factory_attributes->environment, m_environment_del_drawable, attributes->labels[index].drawable, attributes->labels[index].layer,
             e_environment_surface_ui);
...
d_define_class(effecteer) {d_hook_method(effecteer, e_flag_private, get_effect),
