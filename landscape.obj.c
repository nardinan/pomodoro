          ...
          if ((item_json = d_call(factory, m_factory_get_json, string_supply_item))) {
            if ((current_item = (struct s_landscape_item *) d_malloc(sizeof(struct s_landscape_item)))) {
              ...
              if ((current_item->item = f_item_new(d_new(item), string_supply_item))) {
                ...
                d_call(current_item->item, m_drawable_set_center, (0.0 - current_item->position_x),
                       (0.0 - current_item->position_y)); /* reset the zoom-on-ROI */
        ...
        while ((d_call(json, m_json_get_string, &string_supply, "sds", "surfaces", index_layer, "drawable"))) {
          ...
          if ((current_surface = (struct s_landscape_surface *) d_malloc(sizeof(struct s_landscape_surface)))) {
            ...
            if ((current_surface->drawable = d_call(factory, m_factory_get_media, string_supply, &type))) {
              ...
              d_call(current_surface->drawable, m_drawable_set_center, (0.0 - current_surface->offset_x),
                     (0.0 - current_surface->offset_y)); /* reset the zoom-on-ROI */
...
d_define_method(landscape, hide)(struct s_object *self, struct s_object *environment) {
  ...
  d_foreach(&(landscape_attributes->surfaces), current_surface, struct s_landscape_surface)d_call(environment, m_environment_del_drawable,
                                                                                                  current_surface->drawable, current_surface->layer,
...
d_define_method(landscape, stop)(struct s_object *self) {
  d_using(landscape);
  struct s_landscape_track *current_track;
  d_foreach(&(landscape_attributes->tracks), current_track, struct s_landscape_track)if (current_track->track)
...
d_define_method(landscape, play)(struct s_object *self, const char *label) {
  ...
  struct s_landscape_track *current_track;
  d_call(self, m_landscape_stop, NULL);
  d_foreach(&(landscape_attributes->tracks), current_track, struct s_landscape_track)if (f_string_strcmp(current_track->label, label) == 0) {
...
d_define_method(landscape, set_item_solid)(struct s_object *self, const char *label, t_boolean solid) {
  d_using(landscape);
  struct s_landscape_item *current_item;
  d_foreach(&(landscape_attributes->items), current_item, struct s_landscape_item)if (f_string_strcmp(current_item->label, label) == 0)
      d_call(current_item->item, m_item_set_solid, solid);
  return self;
}
...
d_define_method(landscape, get_dimension)(struct s_object *self, double *dimension_w, double *dimension_h) {
  d_using(landscape);
  struct s_landscape_surface *current_surface;
  d_foreach(&(landscape_attributes->surfaces), current_surface, struct s_landscape_surface)if (current_surface->reference) {
...
d_define_method(landscape, validator)(struct s_object *self, struct s_object *entity, double current_x, double current_y, double *new_x, double *new_y,
  ...
  if (entity) {
    ...
    d_foreach(&(landscape_attributes->items), current_item, struct s_landscape_item) {
      ...
      if ((final_distance = d_point_square_distance(entity_position_x, entity_position_y, item_position_x, item_position_y)) < d_item_lost_square_distance)
...
d_define_method(landscape, update)(struct s_object *self, struct s_object *environment) {
  ...
  d_foreach(&(landscape_attributes->surfaces), current_surface, struct s_landscape_surface) {
    position_x =
      ((landscape_attributes->position_x + current_surface->offset_x) + (camera_position_x * (1.0 - current_surface->speed_ratio_x) * (1.0 / ratio_x)));
    position_y = ((landscape_attributes->position_y + current_surface->offset_y) + (camera_position_y * (1.0 - current_surface->speed_ratio_y) * ratio_y));
    d_call(current_surface->drawable, m_drawable_set_position, position_x, position_y);
  }
  current_timestamp = time(NULL);
  d_foreach(&(landscape_attributes->scripts), current_script, struct s_landscape_script)if ((!current_script->first_execution) ||
                                                                                            ((current_script->last_update + current_script->frequency) <
                                                                                             current_timestamp)) {
...
d_define_method(landscape, delete)(struct s_object *self, struct s_landscape_attributes *attributes) {
  ...
  while ((current_item = (struct s_landscape_item *) attributes->items.head)) {
    ...
    if (v_developer_mode) {
      ...
      printf("\t{\"item\":\"%s\", \"position_x\":%.01f, \"position_y\":%.01f, \"zoom\":%.02f}\n", current_item->label, (position_x - attributes->position_x),
             (position_y - attributes->position_y), zoom);
    }
...
d_define_class(landscape) {d_hook_method(landscape, e_flag_public, load),
